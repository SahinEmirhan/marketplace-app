...
export class ChatService {
    ...
    async createChatRoom(productId, userId) {
        ...
        return new ChatResponse(chat.getId(), product.getName(), product.getPrice(), signedImageUrl);
    }
    async getMyChats(userId) {
        const chats = await this.chatRepository.findMyChats(userId);
        const chatProducts = await Promise.all(chats.map((c) => this.productRepository.getProduct(c.getProductId())));
        ...
        for (let i = 0; i < chatProducts.length; i++) {
            ...
            chatsResList.push(new ChatResponse(chats[i]?.getId(), chatProducts[i].getName(), chatProducts[i].getPrice(), signedImageUrl));
        }
        return chatsResList;
    }
